...
def data():
  ...
  if dept_id:
    mf.exact(field_name = 'd ept_id', value = dept_id)
...
def save():
  ...
  if not username or not real_name or not password:
    return fail_api(msg = '账号姓名密码不能为空')
...
def update():
  ...
  role_id = xss_escape(req_json.get('deptId'))
  role_ids = a.split(',')
...
def center():
  ...
  user_logs = AdminLog.query.filter_by(url = '/passpoprt/login').filter_by(uid = current_user.id).order_by(desc(AdminLog.create_time)).limit(10)
  return render_template('admin/user/center.html', user_info = user_info, user_logs = user_logs)

@admin_user.put('/profile')
@login_required
def profile():
